...
use MyModule::Build::Base;

our @ISA = qw(MyModule::Build::Base);
our $VERSION = '0.4229';
$VERSION = eval $VERSION;

# Inserts the given module into the @ISA hierarchy between
# MyModule::Build and its immediate parent
...
if (grep {-e File::Spec->catfile($_, qw(Module Build Platform), $^O) . '.pm'} @INC) {
  __PACKAGE__->_interpose_module("MyModule::Build::Platform::$^O");

} elsif ( my $ostype = os_type() ) {
  __PACKAGE__->_interpose_module("MyModule::Build::Platform::$ostype");
...
MyModule::Build - Build and install Perl modules
...
C<MyModule::Build> is a system for building, testing, and installing
...
system - most of the C<MyModule::Build> code is pure-perl and written in a
very cross-platform way.

See L</"COMPARISON"> for more comparisons between C<MyModule::Build> and
other installer tools.

To install C<MyModule::Build>, and any other module that uses
C<MyModule::Build> for its installation process, do the following:
...
The documentation for C<MyModule::Build> is broken up into sections:

=over

=item General Usage (L<MyModule::Build>)

This is the document you are currently reading. It describes basic
usage and background information.  Its main purpose is to assist the
user who wants to learn how to invoke and control C<MyModule::Build>
scripts at the command line.

=item Authoring Reference (L<MyModule::Build::Authoring>)

This document describes the structure and organization of
C<MyModule::Build>, and the relevant concepts needed by authors who are
writing F<Build.PL> scripts for a distribution or controlling
C<MyModule::Build> processes programmatically.

=item API Reference (L<MyModule::Build::API>)

This is a reference to the C<MyModule::Build> API.

=item Cookbook (L<MyModule::Build::Cookbook>)
...
for details about how MyModule::Build determines where to install
...
If you want to have MyModule::Build find test files with different file
...
Suppresses the check upon startup that the version of MyModule::Build
...
Prints MyModule::Build debugging information to STDOUT, such as a trace of
...
When MyModule::Build starts up, it will look first for a file,
...
When you invoke MyModule::Build's C<build> action, it needs to figure
...
Natively, MyModule::Build provides default installation locations for
...
C<prefix> should be used when you want MyModule::Build to install your
...
MyModule::Build's --prefix logic is equivalent to the PREFIX logic found
...
See L<MyModule::Build::Cookbook/Installing in the same location as
...
A comparison between C<MyModule::Build> and other CPAN distribution installers.
...
C<MyModule::Build> does not, nor do other pure-perl installers following the
F<Build.PL> spec such as L<MyModule::Build::Tiny>. In practice, this is usually
...
C<MyModule::Build> was added to core in Perl 5.10 and removed from core in Perl
5.20, and (like L<ExtUtils::MakeMaker>) is only updated to fix critical issues
and maintain compatibility. C<MyModule::Build> and other non-core installers like
L<MyModule::Build::Tiny> are installed from CPAN by declaring themselves as a
...
customization of the entire build. Customizing C<MyModule::Build> involves
subclassing C<MyModule::Build> itself, adding or overriding pure-perl methods
...
L<MyModule::Build::Tiny> does not allow for customization.

=item *

C<MyModule::Build> provides more features and a better experience for distribution
...
standalone authoring tool for distributions using L<MyModule::Build::Tiny>, which
...
perl(1), L<MyModule::Build::Cookbook>, L<MyModule::Build::Authoring>,
L<MyModule::Build::API>, L<ExtUtils::MakeMaker>
