...
sub write_config_data {
  ...
  print {$fh} $template;
  print {$fh} "\n__DATA__\n";
  print {$fh} MyModule::Build::Dumper->_data_dump([$args{config_data}, $args{feature}, $args{auto_features}]);
  close $fh;
}
...
MyModule::Build::Notes - Create persistent distribution configuration modules

=head1 DESCRIPTION

This module is used internally by MyModule::Build to create persistent
configuration files that can be installed with a distribution.  See
L<MyModule::Build::ConfigData> for an example.
...
sub write {
  ...
  # Can't use MyModule::Build::Dumper here because M::B is only a
...
sub feature {
  ...
  require MyModule::Build;  # XXX should get rid of this
  foreach my $type (sort keys %$info) {
    my $prereqs = $info->{$type};
    next if $type eq 'description' || $type eq 'recommends';

    foreach my $modname (sort keys %$prereqs) {
      my $status = MyModule::Build->check_installed_status($modname, $prereqs->{$modname});
...
C<NOTES_NAME> was automatically created using C<MyModule::Build>.
C<MyModule::Build> was written by Ken Williams, but he holds no
