...
my %makefile_to_build =
  (
   ...
   VERBINST     => 'verbose',
   INC          => sub { map {(extra_compiler_flags => $_)} MyModule::Build->split_like_shell(shift) },
...
sub _merge_prereq {
  ...
  for my $p ( $req, $breq ) {
    for my $k (sort keys %$p) {
      ...
      if ( ! defined $v_obj ) {
          die "A prereq of the form '$p->{$k}' for '$k' is not supported by MyModule::Build::Compat ( use a simpler version like '0.05' or 'v1.4.25' )\n";
      }
...
may be removed in a future version of MyModule::Build in favor of the
'configure_requires' property.  See MyModule::Build::Compat
...
HERE
  ...
  my $subclass_load = '';
  if (ref($build) ne "MyModule::Build") {
  ...
  if ($type eq 'small') {
    ...
    use MyModule::Build::Compat 0.02;
    %s
    MyModule::Build::Compat->run_build_pl(args => \@ARGV);
    require %s;
    MyModule::Build::Compat->write_makefile(build_class => '%s');
EOF

  } elsif ($type eq 'passthrough') {
    printf {$fh} <<'EOF', $subclass_load, ref($build), ref($build);

    unless (eval "use MyModule::Build::Compat 0.02; 1" ) {
      print "This module requires MyModule::Build to install itself.\n";

      require ExtUtils::MakeMaker;
      my $yn = ExtUtils::MakeMaker::prompt
	('  Install MyModule::Build now from CPAN?', 'y');

      unless ($yn =~ /^y/i) {
	die " *** Cannot install without MyModule::Build.  Exiting ...\n";
      }
      ...
      CPAN::Shell->install('MyModule::Build::Compat');
      CPAN::Shell->expand("Module", "MyModule::Build::Compat")->uptodate
	or die "Couldn't install MyModule::Build, giving up.\n";

      chdir $cwd or die "Cannot chdir() back to $cwd: $!";
    }
    eval "use MyModule::Build::Compat 0.02; 1" or die $@;
    %s
    MyModule::Build::Compat->run_build_pl(args => \@ARGV);
...
sub makefile_to_build_args {
  ...
  foreach my $arg (@_) {
    ...
    ( $val ) = MyModule::Build->_detildefy( $val ) if $val =~ /^~/;
...
sub run_build_pl {
  ...
  MyModule::Build->run_perl_script($in{script}, [], \@args) or die "Couldn't run $in{script}: $!";
}

sub fake_makefile {
  my ($self, %args) = @_;
  unless (exists $args{build_class}) {
    warn "Unknown 'build_class', defaulting to 'MyModule::Build'\n";
    $args{build_class} = 'MyModule::Build';
  }
...
sub _is_vms_mms {
  return MyModule::Build->is_vmsish && ($Config{make} =~ m/MM[SK]/i);
}
...
MyModule::Build::Compat - Compatibility with ExtUtils::MakeMaker
...
bone, you can use C<MyModule::Build::Compat> to automatically generate a
F<Makefile.PL> for you, in one of several different styles.

C<MyModule::Build::Compat> also provides some code that helps out the
F<Makefile.PL> at runtime.

=head1 WARNING

Note that C<MyModule::Build::Compat> more often causes installation issues
...
styles, based on the supplied C<MyModule::Build> object C<$build>.  This is
typically controlled by passing the desired style as the
C<create_makefile_pl> parameter to C<MyModule::Build>'s C<new()> method;
...
use C<ExtUtils::MakeMaker> and won't rely on C<MyModule::Build> at all.
...
environment, or if you subclass C<MyModule::Build> to do some
customization, because the vanilla F<Makefile.PL> won't do any of that.
Many standard C<MyModule::Build> features such as C<test_requires> are also
not supported.
...
already have C<MyModule::Build> installed in order to use this, or else
...
This is just like the C<small> option above, but if C<MyModule::Build> is
...
of MyModule::Build.  Modern CPAN.pm and CPANPLUS will recognize the
C<configure_requires> metadata property and install MyModule::Build before
running Build.PL if MyModule::Build is listed and MyModule::Build now
...
C<ExtUtils::MakeMaker> and C<MyModule::Build> accept different arguments, this
...
through to the corresponding C<MyModule::Build> actions.
...
that the user must install C<MyModule::Build> before installing your
module.
...
ever have to install C<MyModule::Build> if they use the F<Makefile.PL>, but
they won't get to take advantage of C<MyModule::Build>'s extra features
either.
...
built using C<MyModule::Build::Compat>.  This will mean that people can
...
L<MyModule::Build>(3), L<ExtUtils::MakeMaker>(3)
