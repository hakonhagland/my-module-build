package MyModule::Build::Cookbook;
use strict;
use warnings;
our $VERSION = '0.4229';
...
MyModule::Build::Cookbook - Examples of MyModule::Build Usage

=head1 DESCRIPTION

C<MyModule::Build> isn't conceptually very complicated, but examples are
...
=head2 Installing modules that use MyModule::Build
...
C<MyModule::Build> relies heavily on various values from perl's
...
you know what you're doing>, you can tell C<MyModule::Build> to pretend
...
the L<MyModule::Build/config> method:
...
Create a MyModule::Build object (or an object of a custom MyModule::Build
...
This is the interface we use to test MyModule::Build itself in the
regression tests.
...
C<ExtUtils::MakeMaker> and C<MyModule::Build>.

    MakeMaker             MyModule::Build
    PREFIX=...            --prefix ...
...
You can install into the same location with MyModule::Build using this:
...
installation locations.  Now that C<MyModule::Build> and C<MakeMaker> both
...
C<MyModule::Build> supports running a single test, which enables you to
...
C<< MyModule::Build->new() >> in the C<Build.PL> script, which can
...
The C<MyModule::Build::Compat> module, which is part of
C<MyModule::Build>'s distribution, is responsible for creating these
F<Makefile.PL>s.  Please see L<MyModule::Build::Compat> for the details.


=head2 Changing the order of the build process

The C<build_elements> property specifies the steps C<MyModule::Build>
...
want to subclass C<MyModule::Build> and create a special method to
process them, named C<process_${kind}_files()>:

  use MyModule::Build;
  my $class = MyModule::Build->subclass(code => <<'EOF');
...
Please note that these examples use some capabilities of MyModule::Build
...
By default, MyModule::Build creates seven subdirectories of the F<blib>
...
during the C<install> action (see L<MyModule::Build/"INSTALL PATHS">.

If you need to create a new custom type of installable element, e.g. C<conf>,
then you need to tell MyModule::Build where things in F<blib/conf/>
...
of MyModule::Build.  They can serve as real-world examples for others.
...
You can subclass C<MyModule::Build> on the fly using the C<subclass()>
method and override the methods that perform the actions.  You may
need to read through C<MyModule::Build::Authoring> and
C<MyModule::Build::API> to find the methods you want to override.  All
...
the C<install> action:
  ...
  $class->new(
      ...
      # rest of the usual MyModule::Build parameters
  )->create_build_script;
...
=head2 Bundling MyModule::Build

Note: This section probably needs an update as the technology improves
(see contrib/bundle.pl in the distribution).

Suppose you want to use some new-ish features of MyModule::Build,
e.g. newer than the version of MyModule::Build your users are likely to
...
MyModule::Build.  See L<MyModule::Build::Authoring>.
...
Next, add this to the top of your F<Build.PL>.
    ...
    # Find out what version of MyModule::Build is installed or fail quietly.
    # This should be cross-platform.
    my $Installed_MB =
        `$^X -e "eval q{require MyModule::Build; print MyModule::Build->VERSION} or exit 1"`;
...
And write the rest of your F<Build.PL> normally.  MyModule::Build will
...
scenario; see C<inc/latest.pm> in the MyModule::Build distribution for
...
perl(1), L<MyModule::Build>(3), L<MyModule::Build::Authoring>(3),
L<MyModule::Build::API>(3)
