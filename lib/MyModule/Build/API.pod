...
MyModule::Build::API - API Reference for Module Authors

=for :stopwords apache bsd distdir distsign gpl installdirs lgpl mit mozilla packlists

=head1 DESCRIPTION

I list here some of the most important methods in C<MyModule::Build>.
Normally you won't need to deal with these methods unless you want to
subclass C<MyModule::Build>.  But since one of the reasons I created
...
C<MyModule::Build> object representing the current build.  You can use
...
during a regression test:
  ...
  # In t/colortest.t:
  use MyModule::Build;
  my $build = MyModule::Build->current;
...
in future releases of MyModule::Build, so please report any anomalies
you find.
...
C<MyModule::Build> was invoked from.

=item new()

[version 0.03]

Creates a new MyModule::Build object.  Arguments to the new() method are
...
This parameter determines whether MyModule::Build will add itself
automatically to configure_requires (and build_requires) if MyModule::Build
...
The MyModule::Build class or subclass to use in the build script.
Defaults to "MyModule::Build" or the class name passed to or created by
a call to L</subclass()>.  This property is useful if you're
writing a custom MyModule::Build subclass and have a bootstrapping
...
See the documentation for L<MyModule::Build::Authoring/"PREREQUISITES">
...
This might be a specific minimum version of C<MyModule::Build> or any
...
MyModule::Build may automatically add itself to configure_requires.
...
If true, this parameter tells MyModule::Build to create a F<.packlist>
...
MyModule::Build version 0.2609; previously no packlists were ever
created by MyModule::Build.
...
given module.  C<MyModule::Build> (or some higher-level tool) will
...
This parameter tells MyModule::Build to automatically create a
...
This parameter lets you use C<MyModule::Build::Compat> during the
...
should be one of the styles named in the L<MyModule::Build::Compat>
documentation.
...
This parameter tells MyModule::Build to automatically create a F<README>
...
then C<MyModule::Build> looks in the POD of the module from which it gets
...
PPD files.  If this is not specified, then C<MyModule::Build> looks at
...
Currently C<MyModule::Build> doesn't actually do anything with this flag
...
F<Build>, and they will be stored in the MyModule::Build object and can
...
specifications before they're passed on to C<MyModule::Build>'s own
argument processing.
...
B<WARNING:> Any option specifications that conflict with MyModule::Build's own
options (defined by its properties) will throw an exception.  Use capitalized
option names to avoid unintended conflicts with future MyModule::Build options.
...
L<MyModule::Build/"INSTALL PATHS">
...
As of MyModule::Build version 0.36_14, you may use a L<Software::License>
...
indicate that so that MyModule::Build can properly handle these created
files:
...
in a "normal" MyModule::Build-style distribution.  This parameter is
...
they would be found in a "normal" MyModule::Build-style distribution.
...
Normally, C<MyModule::Build> does not search subdirectories when looking
...
One note: currently C<MyModule::Build> doesn't actually I<require> the
...
call C<resume()> to return the resulting C<MyModule::Build> object to the caller.
...
C<MyModule::Build> object is created using the C<resume()> method to
...
This creates a new C<MyModule::Build> subclass on the fly, as described
in the L<MyModule::Build::Authoring/"SUBCLASSING"> section.  The caller
...
Adds a property to a MyModule::Build class. Properties are those attributes of a
MyModule::Build object which can be passed to the constructor and which have
...
MyModule::Build class, and an accessor will be built to allow the property to be
...
See also
L<MyModule::Build::Cookbook/"Adding new file types to the build process">.
...
C<MyModule::Build> that certain files should be removed when the user
...
command line options to F<Build.PL> or F<Build>, minus the MyModule-Build
specific options.
...
(C<< MyModule::Build->check_installed_status($module, $version) >>).
...
Configuration values set in this way using the MyModule::Build object
...
MyModule::Build's main support for configuration of installed modules.
See also L<MyModule::Build::Authoring/"SAVING CONFIGURATION INFORMATION">.
...
MyModule::Build it's conceivable that this run-only-once mechanism will
...
actions, e.g. by applications controlling MyModule::Build-based builds
...
Features set in this way using the MyModule::Build object will be
...
C<new()> method is run, if the MyModule::Build Perl API is being used
...
If you're subclassing MyModule::Build and some code needs to alter its
...
C<new()> method is called, if the MyModule::Build Perl API is being used instead
...
If you would like to add other useful metadata, C<MyModule::Build>
...
L<MyModule::Build::Cookbook>(3), L<ExtUtils::MakeMaker>(3)
