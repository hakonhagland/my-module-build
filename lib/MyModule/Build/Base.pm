...
use File::Spec 0.82 ();
use File::Compare ();
use MyModule::Build::Dumper ();
use Text::ParseWords ();

use Module::Metadata;
use MyModule::Build::Notes;
use MyModule::Build::Config;
use version;
...
sub resume {
  ...
  # If someone called MyModule::Build->current() or
  # MyModule::Build->new_from_context() and the correct class to use is
  # actually a *subclass* of MyModule::Build, we may need to load that
...
DIEFATAL
  ...
  unless ($self->allow_mb_mismatch) {
    my $mb_version = $MyModule::Build::VERSION;
    if ( $mb_version ne $self->{properties}{mb_version} ) {
      $self->log_warn(<<"MISMATCH");
* WARNING: Configuration was initially created with MyModule::Build
...
sub _construct {
  ...
  my $self = bless {
      args => {%$args},
      config => MyModule::Build::Config->new(values => $config),
      properties => {
          base_dir        => $package->cwd,
          mb_version      => $MyModule::Build::VERSION,
          %input,
      },
      phash => {},
  ...
  foreach (qw(notes config_data features runtime_params cleanup auto_features)) {
    ...
    $ph->{$_} = MyModule::Build::Notes->new(file => $file);
...
sub _discover_perl_interpreter {
  my $proto = shift;
  my $c     = ref($proto) ? $proto->{config} : 'MyModule::Build::Config';
...
sub _mb_feature {
  ...
  if (($self->module_name || '') eq 'MyModule::Build') {
    # We're building MyModule::Build itself, so ...::ConfigData isn't
    # valid, but $self->features() should be.
    return $self->feature(@_);
  } else {
    require MyModule::Build::ConfigData;
    return MyModule::Build::ConfigData->feature(@_);
  }
}
...
sub ACTION_config_data {
  ...
  MyModule::Build::Notes->write_config_data
    (
     file => $notes_pm,
     module => $module_name,
...
__PACKAGE__->add_property(build_class => 'MyModule::Build');
...
sub config {
  my $self = shift;
  my $c = ref($self) ? $self->{config} : 'MyModule::Build::Config';
...
sub mb_parents {
    ...
    while (@in_stack) {
        next unless defined($current = shift @in_stack)
          && $current->isa('MyModule::Build::Base');
        push @out, $current;
        next if $current eq 'MyModule::Build::Base';
        no strict 'refs';
        unshift @in_stack,
          map {
...
# XXX Problem - if MyModule::Build is loaded from a different directory,
...
sub _pod_parse {
  ...
  require MyModule::Build::PodParser;
  my $parser = MyModule::Build::PodParser->new(fh => $fh);
  my $method = "get_$part";
...
sub read_config {
  ...
  $self->{config} = MyModule::Build::Config->new(values => $c);
}
...
sub _write_data {
  ...
  print {$fh} MyModule::Build::Dumper->_data_dump($data);
  close $fh;
}
...
sub auto_config_requires {
  ...
  if ( $self->dist_name ne 'MyModule-Build'
    ...
    $self->log_warn(<<EOM);
MyModule::Build was not found in configure_requires! Adding it now
automatically as: configure_requires => { 'MyModule::Build' => $ver }
EOM
    $self->_add_prereq('configure_requires', 'MyModule::Build', $ver);
  }
...
sub print_build_script {
  ...
  if ( -f $self->metafile ) {
    ...
    $config_requires = $meta && $meta->{prereqs}{configure}{requires}{'MyModule::Build'};
  }
  $config_requires ||= 0;
...
use $build_package;
MyModule::Build->VERSION(q{$config_requires});

# Some platforms have problems setting \$^X in shebang contexts, fix it up here
\$^X = MyModule::Build->find_perl_interpreter;
...
sub create_mymeta {
  ...
  if ($meta_obj) {
    ...
    my %updated = (
      ...
      dynamic_config => 0,
      generated_by => "MyModule::Build version $MyModule::Build::VERSION",
    );
...
sub read_args {
  ...
  if ($args{makefile_env_macros}) {
    require MyModule::Build::Compat;
    %args = (%args, MyModule::Build::Compat->makefile_to_build_macros);
  }
...
# merge MyModule::Build argument lists that have already been parsed
...
# merge the relevant options in ~/.modulebuildrc into MyModule::Build's
...
sub ACTION_prereq_data {
  my $self = shift;
  $self->log_info( MyModule::Build::Dumper->_data_dump( $self->prereq_data ) );
}
...
EOF
  ...
  print "See `perldoc MyModule::Build` for complete documentation.\n";
}
...
sub ACTION_testpod {
  ...
  { package # hide from PAUSE
      MyModule::Build::PodTester;  # Don't want to pollute the main namespace
...
sub htmlify_pods {
  ...
  foreach my $pod ( sort keys %$pods ) {
    ...
    if ( $with_ActiveState ) {
      ...
      my $abstract = MyModule::Build::PodParser->new(fh => $fh)->get_abstract();
...
sub ACTION_ppd {
  ...
  require MyModule::Build::PPMMaker;
  my $ppd = MyModule::Build::PPMMaker->new();
...
sub _sign_dir {
  ...
  # Add SIGNATURE to the MANIFEST
  {
    ...
    $self->_add_to_manifest($manifest, "SIGNATURE    Added here by MyModule::Build");
  }
...
sub do_create_makefile_pl {
  ...
  eval { MyModule::Build::Compat->create_makefile_pl($self->create_makefile_pl, $self, @_) };
  if ( $@ ) {
...
# Avoid MyModule::Build generated and utility files.
\bBuild$
\bBuild.bat$
\b_build
\bBuild.COM$
...
MyModule::Build::Base - Default methods for MyModule::Build

=head1 SYNOPSIS

  Please see the MyModule::Build documentation.

=head1 DESCRIPTION

The C<MyModule::Build::Base> module defines the core functionality of
C<MyModule::Build>.  Its methods may be overridden by any of the
platform-dependent modules in the C<MyModule::Build::Platform::>
...
Please see the C<MyModule::Build> documentation for more details.
