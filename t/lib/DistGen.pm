...
sub new_from_context {
  ...
  return MyModule::Build->new_from_context( quiet => 1, @args );
}

sub run_build_pl {
  my ($self, @args) = @_;
  require MyModule::Build;
  return MyModule::Build->run_perl_script('Build.PL', [], [@args])
}
...
sub run_build {
  ...
  return MyModule::Build->run_perl_script($build_script, [], [@args])
}
...
of file C<$content>, it takes a hash-ref of MyModule::Build constructor
arguments:
...
These use MyModule::Build->run_perl_script() to ensure that Build.PL or Build are
run in a separate process using the current perl interpreter.  (MyModule::Build
