...
blib_load('MyModule::Build::ConfigData');
...
elsif ( MyModule::Build::ConfigData->feature('inc_bundling_support') ) {
  plan tests => 19;
} else {
...
my $current_mb = MyModule::Build->resume();
...
is_deeply( $mb->bundle_inc, [ 'MyModule::Build' ],
  "MyModule::Build is flagged for bundling"
);
...
HERE
  ...
  isa_ok( $mb, "MyModule::Build" );
  is_deeply( [sort @{$mb->bundle_inc}], [ 'Foo', 'MyModule::Build' ],
    "MyModule::Build and Foo are flagged for bundling"
  );
...
HERE
  ...
  $mb = $dist->new_from_context(); # quiet by default
  isa_ok( $mb, "MyModule::Build" );
  is_deeply( [sort @{$mb->bundle_inc}], [ 'Bar', 'MyModule::Build' ],
    "MyModule::Build and Bar are flagged for bundling"
  );
