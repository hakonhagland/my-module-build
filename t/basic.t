...
{
  my $mb = MyModule::Build->new( module_name => $dist->name );
  ok $mb;
  is $mb->module_name, $dist->name;
  is $mb->build_class, 'MyModule::Build';
  is $mb->dist_name, $dist->name;

  $mb = MyModule::Build->new( dist_name => $dist->name, dist_version => 7 );
  ok $mb;
...
{
  my %actions = map {$_, 1} MyModule::Build->known_actions;
  ok $actions{clean};
  ok $actions{distdir};
}
...
# Test prerequisite checking
{
  ...
  my $info = MyModule::Build->check_installed_status('File::Spec', 0);
  ok $info->{ok};
  is $info->{have}, $File::Spec::VERSION;

  # Make sure check_installed_status() works with an advanced spec
  $info = MyModule::Build->check_installed_status('File::Spec', '> 0');
  ok $info->{ok};
  ...
  $info = MyModule::Build->check_installed_status('Foo::Module', '1.01_02');
...
# Getopt::Long specs work as expected.
{
  ...
  $dist->regen;
  eval {MyModule::Build->run_perl_script('Build.PL', [], ['--nouse-rcfile', '--config', "foocakes=barcakes", '--foo', '--bar', '--bar', '-bat=hello', 'gee=whiz', '--any', 'hey', '--destdir', 'yo', '--verbose', '1'])};
  is $@, '';

  my $mb = MyModule::Build->resume;
...
# Test conversion of shell strings
{
  ...
  eval {MyModule::Build->run_perl_script('Build.PL', [], ['--extra_compiler_flags', '-I/foo -I/bar',
							'--extra_linker_flags', '-L/foo -L/bar'])};
  $mb = MyModule::Build->resume;
  ok $mb;
  is_deeply $mb->extra_compiler_flags, ['-I/foo', '-I/bar'], "Should split shell string into list";
  is_deeply $mb->extra_linker_flags,   ['-L/foo', '-L/bar'], "Should split shell string into list";
}
...
# Test include_dirs.
{
  ...
  # Try again with command-line args
  eval { MyModule::Build->run_perl_script(
      'Build.PL', [],
      ['--include_dirs', '/foo', '--include_dirs', '/bar' ],
  ) };

  ok $mb = MyModule::Build->resume;
  is_deeply $mb->include_dirs, ['/foo', '/bar'], 'Should have include dirs';
