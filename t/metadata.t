...
{
  my $mb_prereq = { 'MyModule::Build' => $MyModule::Build::VERSION };
  my $mb_config_req = {
    'MyModule::Build' => sprintf '%2.2f', int($MyModule::Build::VERSION * 100)/100
  };
  my $node;
  my $output = stdout_stderr_of( sub {
    $node = $mb->get_metadata( auto => 1 );
  });
  like( $output, qr/MyModule::Build was not found in configure_requires/,
  ...
  like $node->{generated_by}, qr{MyModule::Build};
