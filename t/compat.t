...
stdout_stderr_of( sub {
    $mb = MyModule::Build->new_from_context;
});
ok $mb, "MyModule::Build->new_from_context";
...

{
  ...
  my $maketext = eval { MyModule::Build::Compat->fake_makefile(makefile => $makefile) };
...

{
  ...
  local @Foo::Builder::ISA = qw(MyModule::Build);
  my $foo_builder;
  stdout_stderr_of( sub {
  ...
  my $bar_builder;
  stdout_stderr_of( sub {
    $bar_builder = MyModule::Build->subclass( class => 'Bar::Builder' )->new_from_context;
  });
  foreach my $style ('passthrough', 'small') {
...

{
  ...
  my $new_build = MyModule::Build->resume();
...

{
  ...
  my $mb;
  stdout_stderr_of( sub {
      $mb = MyModule::Build->new_from_context( recursive_test_files => 1 );
  });
...
sub create_makefile_pl {
    my @args = @_;
    stdout_stderr_of( sub { MyModule::Build::Compat->create_makefile_pl(@args) } );
